...
['tan','NURAA UV Shield Sunscreen SPF 40','Sunscreen','Niacinamide 2% · Zinc Oxide · Licorice'],
['tan','NURAA De-Tan Daily Sunscreen SPF 50','Sunscreen','Zinc Oxide 5% · Licorice Extract · Aloe Vera'],
['tan','NURAA Ultra Defense Sunscreen SPF 60','Sunscreen','Titanium Dioxide · Zinc Oxide · Niacinamide 3%'],
...
['blackheads','NURAA Pore Refining BHA Face Serum','Face Serum','Salicylic Acid 1% · Zinc PCA · Green Tea'],

['glow','NURAA Saffron & Honey Face Cleanser','Face Wash','Saffron Extract · Honey · Vitamin C'],
['glow','NURAA Vitamin C Radiance Face Serum','Face Serum','Vitamin C 1% · Saffron Extract · Honey'],
['glow','NURAA Golden Saffron Body Lotion','Body Lotion','Saffron Extract · Sandalwood Oil · Shea Butter'],
...
// Map clean file names to each of the 16 products with smart fallback for catalog rendering
function getProductFileName(name, type, category) {
    if (name === "NURAA De-Tan Bathing Bar" || name === "NURAA Velvet Rose & Shea Soap") {
        return "kojic_detan_soap.jpg";
    }
    if (name === "NURAA Charcoal Detox Soap") {
        return "charcoal_detox_soap.jpg";
    }
    if (name.includes("Serum") || type === "Face Serum") {
        return "bright_reveal_serum.jpg";
    }
    if (name.includes("Sunscreen") || name.includes("UV Shield")) {
        return "natural_shield_sunscreen.jpg";
    }
    if (name.includes("Lotion")) {
    ...
    if (type === "Face Serum" || name.includes("Serum")) {
        return "bright_reveal_serum.jpg";
    }
    if (type === "Sunscreen" || name.includes("Shield")) {
        return "natural_shield_sunscreen.jpg";
    }
...
// Generate fallback descriptions & specs dynamically for all 16 products
...
function renderPdp(){
    ...
    if(!el)return;
    const name=new URLSearchParams(location.search).get('name')||products[0][1];
    const p=products.find(x=>x[1]===name)||products[0];
    const spec=getPdpSpecs(p[1], p[0], p[2], p[3]);
